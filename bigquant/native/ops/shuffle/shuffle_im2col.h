...
template <size_t shuffle_rows, size_t shuffle_cols, LAYOUT layout>
void PadQuantizeShuffleIm2colRef(float *data, size_t batch_size,
                                 ...
                                 uint8_t *data_col, float *min, float *max, float *ratio, float sw_threshold = 255.0f) {
  ...
  for (size_t batch = 0; batch < batch_size; ++batch) {
    ...
    QuantizeIm2col<float, layout>(data + src_index, channels, height, width, kernel_h, kernel_w, pad_h, pad_w, stride_h,
...
template <size_t shuffle_rows, size_t shuffle_cols>
void PadQuantizeShuffleNCHWIm2col(float *data, size_t batch_size, size_t channels_per_group, size_t groups,
                                  size_t height, size_t width, size_t kernel_h, size_t kernel_w, size_t pad_h,
                                  size_t pad_w, size_t stride_h, size_t stride_w, size_t dilation_h, size_t dilation_w,
                                  uint8_t *data_col[], float *min[], float *max[], float *ratio[],
                                  float sw_threshold) {
  size_t output_h = GetConvOutSize(height, kernel_h, stride_h, pad_h, dilation_h);
  size_t output_w = GetConvOutSize(width, kernel_w, stride_w, pad_w, dilation_w);
  ...
  std::vector<float *> min_per_channel(groups);
  std::vector<float *> max_per_channel(groups);
  for (size_t g = 0; g < groups; ++g) {
    // fix me, hard code is too bad
    aligned_malloc(reinterpret_cast<void **>(&min_per_channel[g]), 64, sizeof(float) * batch_size * height * width);
    aligned_malloc(reinterpret_cast<void **>(&max_per_channel[g]), 64, sizeof(float) * batch_size * height * width);
  }
  FindMinMaxAlongChannel<float, NCHW>(data, groups, min_per_channel.data(), max_per_channel.data(), batch_size,
...
template <size_t shuffle_rows, size_t shuffle_cols, typename findextreme_function, typename quantizekernel_function>
void PadQuantizeShuffleNHWCIm2col(float *data, size_t batch_size, size_t channels_per_group, size_t groups,
                                  size_t height, size_t width, size_t kernel_h, size_t kernel_w, size_t pad_h,
                                  size_t pad_w, size_t stride_h, size_t stride_w, size_t dilation_h, size_t dilation_w,
                                  uint8_t *data_col[], float *min[], float *max[], float *ratio[], float *workspace,
                                  float sw_threshold,
          ...
          */
          ...
          for (size_t h = 0; h < kernel_h; ++h) {
            ...
            if ((dilation_w == 1) && valid_row && (groups == 1) && x_ge_0_and_x_lt_bound(conv_window_x, width) &&
              ...
              float *src_base = data + src_index + z;
              uint8_t *dst_base = addr + shuffle_offset_in_row;
              quantizekernel(dst_base, src_base, scale, shift, total_kernel, shuffle_rows * shuffle_cols, shuffle_cols);
              ...
              for (size_t w = 0; w < kernel_w; ++w) {
                ...
                if (valid_row && valid_col) {
                  ...
                  if (channels_per_group < shuffle_cols) {
                    ...
                    float *src_base = data + src_index + c;
                    uint8_t *dst_base = addr + shuffle_offset_in_row;
                    quantizekernel(dst_base, src_base, scale, shift, total_kernel, shuffle_rows * shuffle_cols, shuffle_cols);
                    shuffle_offset_in_row += total_kernel * shuffle_rows * shuffle_cols;
...
typedef void (*quantize_function)(uint8_t *, float *, const float, const float, const size_t, const size_t, const size_t);

template <LAYOUT layout, size_t shuffle_rows, size_t shuffle_cols, quantize_function quantize>
void PadQuantizeShuffleIm2colWrapper(float *data, size_t batch_size, size_t channels_per_group, size_t groups,
                                     size_t height, size_t width, size_t kernel_h, size_t kernel_w, size_t pad_h,
                                     size_t pad_w, size_t stride_h, size_t stride_w, size_t dilation_h,
                                     size_t dilation_w, uint8_t *data_col[], float *min[], float *max[], float *ratio[],
                                     float *workspace, float sw_threshold, bool layout_transform) {
  if (layout == NCHW) {
    PadQuantizeShuffleNCHWIm2col<shuffle_rows, shuffle_cols>(data,
        batch_size, channels_per_group, groups, height, width, kernel_h, kernel_w, pad_h, pad_w, stride_h,
        stride_w, dilation_h, dilation_w, data_col, min, max, ratio, sw_threshold);
  } else {
    if (layout_transform == false) {
      PadQuantizeShuffleNHWCIm2col<shuffle_rows, shuffle_cols>(data,
          batch_size, channels_per_group, groups, height, width, kernel_h, kernel_w, pad_h, pad_w, stride_h,
          stride_w, dilation_h, dilation_w, data_col, min, max, ratio, NULL,
          sw_threshold, FindMinMaxAlongChannel<float, NHWC>, quantize);
    } else {
      bool memory_allocated = false;
      if (workspace == NULL) {
        aligned_malloc(reinterpret_cast<void **>(&workspace), 64,
                       batch_size * groups * channels_per_group * height * width * sizeof(float));
        memory_allocated = true;
      }
      PadQuantizeShuffleNHWCIm2col<shuffle_rows, shuffle_cols>(data, batch_size, channels_per_group, groups, height, width, kernel_h, kernel_w, pad_h, pad_w, stride_h,
          stride_w, dilation_h, dilation_w, data_col, min, max, ratio, workspace,
          sw_threshold, FindMinMaxAlongChannelThenTranspose<float, NHWC>, quantize);
